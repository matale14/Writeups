...
General information
Scoring flags are in the form PST{.*}, For example PST{someTextandSomeNumbers}.
...
```
It is still possible to respond to this year's employee survey. Feel free to praise me to the skies, so that I appear as a 
...
This unlocked Egg#4 These are called Utmerkelser, or commendations in English and are represented with stars on the Leaderboard.
...
def shift(melding):
    lowercase_alphabet = string.ascii_lowercase   # The characters we are shifting
    plaintext = ''                                # We need to create this before the loop, as we are adding to it
    shift = 0
    while shift < len(lowercase_alphabet):        #Loop goes for as long as the alphabet
        for c in melding:
            index = lowercase_alphabet.find(c.lower()) # we find the position of the first character in the alphabet.
            if index != -1:                            # This makes sure we aren't shifting characters not in the alphabet.
                isupper = c.isupper()                  # Check if it is uppercase or not, we do lowercase by default
                c = lowercase_alphabet[(shift + index) % len(lowercase_alphabet)] # Simply take the original character index and add the shift amount.
                if isupper:
                    c = c.upper()                 # If it's originally uppercase, we make it uppercase.
            plaintext += c                        # Add it

        if 'PST' in plaintext:               # This is the crib. We print the message and shift amount if we find the known text in there.
            print(plaintext)
            print(shift)
            pass
        if 'pst' in plaintext:
        ...
        plaintext = ""       # Reset the plaintext back to nothing
...
Cupcake.png is a high-resolution image that looks very low-res. W

hile kladd.txt is a text file containing 8 sleigh emojis with an encoded string.
...
As everyone knows, Christmas lasts until Easter(A common Norwegian phrase), and here the problems begin...
...
The system we use to find out when it's Easter is behaving strangely, so this important work has now been put on hold. Can you figure out what's wrong?
...
The calculations for this can be seen in 

### FunctionPaaskeAften.sql:
...
Instead of going through the sql calculations and fixing it, I created my own in python (Found the algorithm online, and imported it to python.):
...
print(log['Name'].value_counts())  # Print every unique name, and how many times it appears
```
...
The first byte of input is a number N that represents the number of following bytes of input. 

Sum the following N numbers and write the result


```
LES r10        ; Read number of bytes
SETT r11, 1    ; Increment constant

foodLoop:
LIK r5, r10    ; Have we gone through all bytes?
BHOPP end      ; Then end
PLUSS r5, r11  ; Increment counter

LES r2         ; Read byte
PLUSS r3, r2   ; Pluss r3 with the new number

HOPP foodLoop  ; Jump back until we are out of bytes
...
This can be done easily by just throwing the hex for "Hello World!" and a newline into a .DATA label, and reading it.

```
SETT r3, 0x01 ; Incrementer
FINN message  ; Set address to the data
HOPP loop     ; Jump to the writing loop

message: 
.DATA 0x48,0x65,0x6c,0x6c,0x6f,0x2c,0x20,0x57,0x6f,0x72,0x6c,0x64,0x21,0x0a,0x00

loop:
LAST r2       ; Load byte from memory
PLUSS r0, r3  ; Increment memory address
LIK r2, r4    ; Check if we loaded 00
BHOPP end     ; If we did, we are done
SKRIV r2      ; Write the byte
HOPP loop     ; jump back to the start of the loop
...
If we look at the URL of the slede8 webpage:
...
## December 8th
```
Professional development is important, even now in the run-up to Christmas. Today's topic is ASN.1. Here is some brain exercise from Nissens Kompetansebank ™.
...
ASN is an acronym for Abstract Syntax Notation. And it is a type of notation for describing abstract types and values. So we can use ASN.1 to create any type of encoding and data structures we want to. 
...
flag.reverse()
result = ''.join(str(v) for v in flag)  # Had to reverse it
...
Magic numbers, Magic bytes, or file signatures are the bytes used to identify or verify the contents of a file. For example, the magic numbers for a zip file is `50 4B 05 06` Which when decoded from hex looks like `PK..`. So if you see `PK..` while looking at the hex-dump of something, it's very likely that there is a zip file there.

And in this case, the magic numbers for gzip is `1F8B`.
...
for c in hexmas:                  # Each emoji is just correlated with the value.
    print(hexdict[c], end="")     # So we print the value once we see the emoji
```
...
ME r10, r14     ; Checg if over 256
BHOPP over256

MEL r10, r5     ; Check if Less or equal to 255
BHOPP write_num
...
write_num:
SETT r12, 0x09  ; Checking how large the number is
MEL r10, r12
BHOPP under10
SETT r12, 0x14
ME r10, r12
...
under10:
SETT r12, 0x30 ; Add 0x30 to the number to get the ASCII value and write it.
PLUSS r10, r12
SKRIV r10
STOPP
under20:
...
HOPP write_num
under10_100:
SETT r12, 0x30     ; Encountered a problem if the number was over 100 or 200, but under 110, 210.
SKRIV r12
HOPP write_num
```
...
ULIK r10, r7    ; Check that the first number is not 0
BHOPP notZero

ULIK r13, r7    ; Check that the second number is ALSO not 0
BHOPP notZero
SETT r7, 0x30
SKRIV r7        ; Guess we just write 0
STOPP

notZero:
SETT r14, r10   ; Backup variables
SETT r15, r13
SETT r12, 0x64 
ME r10, r12     ; Check if A < 100
BHOPP checkB
SETT r12, 0xC8  
ME r10, r12     ; Check if A < 200
BHOPP over100
HOPP over200   ; A is over 200


over100:
SETT r12, 0x64  
MINUS r10, r12  ; A is over 100, so we subtract 100
SETT r3, r10    ; r3 holds the remainder
SETT r0, 0x01   ; r0 holds the 1 from 100
HOPP checkB

over200:
SETT r12, 0xC8  ; Same deal
MINUS r10, r12
SETT r3, r10
SETT r0, 0x02
HOPP checkB
...
checkB:
SETT r12, 0x64 ; Do the same with B
ME r13, r12
BHOPP tens
SETT r12, 0xC8
ME r13, r12
...
tens:
PLUSS r10, r13    ; Pluss the remainders
SETT r12, 0x64    ; If the remainders are over 100
ME r10, r12       ; we have to carry that 1 over
BHOPP itWasUnder

SETT r6, 0x01     ; We carry it over
PLUSS r0, r6
MINUS r10, r12    ; And remove 100 from the remainder

itWasUnder:
LIK r0, r7        ; If there is no carryover at all, we just write
BHOPP write_num
SETT r12, 0x30    ; Now we write the 100 decimal
PLUSS r0, r12
SKRIV r0
SETT r12, 0x09
MEL r10, r12      ; Check if we have <10 remaining
BHOPP under10_100b
HOPP write_num
under10_100b:
SETT r12, 0x30
...
write_num:
SETT r12, 0x09   ; Check what our number is
MEL r10, r12
BHOPP under10
SETT r12, 0x14
ME r10, r12
...
under10:
SETT r12, 0x30   ; Add 0x30 to make it ASCII
PLUSS r10, r12
SKRIV r10
STOPP
under20:
...
slemme = []  # Create empty lists
snille = []
with open('liste/slemme.csv', 'r',encoding='utf-8') as f:
    for line in f:
        slemme.append(line)                              # Add each row to list


with open('liste/snille.csv', 'r',encoding='utf-8') as f:
    for line in f:
        snille.append(line)                              # Add each row to list


for barn in slemme:
    barn = barn.split(",")                               # Split first name, last name, and hash
    s = barn[0]+barn[1]                                  # Get the name 
    barn[2] = barn[2].rstrip("\n")                       #strip newline so it doesn't ruin the md5sum
    has = hashlib.md5(s.encode('utf-8')).hexdigest()     # md5sum the name
    if barn[2] != has:                                   # If the original hash does not match our hash
        print(barn[0], barn[1])                          # We found our culprit
        print(barn[2], has)

for barn in snille:                                      # Same on the other table 
    barn = barn.split(",")
    s = barn[0]+barn[1]
    barn[2] = barn[2].rstrip("\n")
...
#This is just a rewrite of the runtime.ts code. I can't be bothered to comment this entire thing.
...
So I entered some code in the ELLER/OR operation to return "wrong" if r8 != 0. Same place that I added `x = 0 ^ regs[3] ^ regs[7]` etc. earlier
...
I then counted the occurrences of each character and discovered something. Half of the characters were used around 30-50 times, while the other half were used 160-200 times. So I tried removing both groups. Nothing. 

![](images/luke13_3.png)


Then it hit me later, and I replaced the groups with spaces. And I got presented with the flag.
...
for k in counts:
    print(k, ":", counts[k])  #Prints the amount each character appears


dem = "7F51EC23"
nym = melding
for c in dem:
    nym = nym.replace(c, " ")  #Replace all of these characters with space

print(nym) #Print the new message
...
User Zup on the Discord also linked this CTF: https://github.com/tghack/tg19hack/blob/master/misc/counter_play/writeup/writeup.md

We can actually see that all the characters that makes up the flag are characters with "holes" in them. and therefore we can just select the white background behind the letters and make it black. Here I just selected the white background of the screenshot with the magic wand tool in photoshop set to 99 tolerance:
...
That was not easy to see right away.
```
...
```
SETT r1, 0x01    ; Set address to 256
HOPP foodLoop    ; Start reading


incMem: 
SETT r15, 0xff   ; Use r15 to check if r0 address is 255
LIK r0, r15
BHOPP over255    ; If it is we have to add 1 to r1, and reset r0 to 0
SETT r15, 0x01
PLUSS r0, r15 
RETUR 
over255:
...
decMem:
SETT r15, 0x00  ; Use r15 to check if r0 address is 0
LIK r0, r15
BHOPP under255  ; If it is we have to subtract 1 from r1, and set r0 to 255
SETT r15, 0x01
MINUS r0, r15 
RETUR
under255:
...
foodLoop:
LES r2          ; Read from the input
SETT r15, 0x00 
LIK r2, r15     ; Check if the input was 0
BHOPP write     ; If it was we stop reading
LAGR r2         ; Store the number
TUR incMem      ; Increment the memory address, TUR/RETURN allows us to return here once we have incremented
HOPP foodLoop   ; Go back and start again

write:
SETT r15, 0x01
MINUS r0, r15   ; Go back in memory, this makes it so we dont start writing the 00
writeLoop:
LAST r2         ; Load the number
SETT r15, 0x00
LIK r2, r15     ; Check if it is 0
BHOPP end       ; End in that case
SKRIV r2        ; Write it
TUR decMem      ; Decrement memory
HOPP writeLoop  ; Start again
...
```
SETT r0, 0x00           ; Setting up constants for incrementing or comparisons
SETT r1, 0x02
SETT r11, 0x01
SETT r12, 0xFF
SETT r13, 0x02
SETT r14, 0
LES r15                 ; Read the length of the input
SETT r10, r15           ; Setting the length to r10 so we can count it more times

LIK r15, r14            ; Checking if the input is 0 length.
BHOPP end
...
nyboble:
SETT r0, 0              ; Set r0 to 0 so we can start swapping
SETT r10, r15
bubble:
MINUS r10, r11          ; Count down the length of the input
LIK r10, r14
BHOPP sjekker           ; Check that we are not at 0
LAST r2                 ; Load number
PLUSS r0, r11           ; Increase mem
LAST r3                 ; Load next number
SE r2, r3               ; If the first number is larger than the second
BHOPP swap              ; We swap
HOPP bubble             ; And back to the loop

swap:          
LAGR r2                 ; Our memory is already set to the next number, so we can just store here
MINUS r0, r11
LAGR r3                 ; Decrement memory and store the second number there
PLUSS r0, r11
LIK r10, r14            ; Check that we are not at 0 more numbers
BHOPP sjekker
HOPP bubble             ; Next number

sjekker:
SETT r0, 0              ; Reset the memory and counter
SETT r10, r15      
sjekkLoop:
MINUS r10, r11          ; Essentially just a copied version of the above bubble sort
LIK r10, r14            ; Very bad code, don't copy this. :)
BHOPP skrivern
...
I forgot how terrible my sorting algorithm was. This is not the code to be learning from :D
...
foo = asn1tools.compile_files('ETSI232-1.txt', 'der') #Compile the specs
...
pretty(encoded) #This lets me see the layout of the message.
...
for k in contents:
    #print(k['cCContents'][1]) #Used this for debugging
    melding += (k['cCContents'][1])

print()  #Used this for debugging
...
Version 1 is the most common, and uses time-based sources to secure randomness. Version 2 is the same, but the least significant bit of the clock sequence are replaced by a system-specific identifier.

Version 3 and 5 use name or namespace hashes to secure randomness.

Version 4 randomizes 122 of the 128-bits, which makes it very unique.
...
Instead of me writing a poorly worded explanation, I can highly reccommend this video: https://www.youtube.com/watch?v=kkMps3X_tEE to explain the algorithm.


Shamir's algorithm also uses Finite/Galois fields, which is what the favourite number would represent. However I did not need it.

I have seen multiple algorithms that do require the favourite number, which is a prime. I believe this makes it so the lagrange polynomials only have a single solution.

```
Now there will be packages for Christmas this year too!
...
The easiest tell for Base64 encoding are the `=` at the end of the line. Base64 splits the input data into chunks of 6 bits. If the last chunk of input data is not 6 long, base64 adds `=` as padding until it reaches the 6 bits.

Base64 has it's name because it is built from 64 characters. It uses some of the most common characers to most encodings. This make sure the data is unlikely to be modified in transit. 
...
fin = splittern[0] # line we want to finish

fin2 = "01010000010100110101010001111011" # We already know this

guide = splittern[1] # Line below our line
...
# We are using two digits from the top line, and one from the line below
# Example: If the top line is 11
# Then there are two possibilites, 110, and 111
# this depends on the bottom line character right below.
...
This includes code to use as well. Most comments are mine, I did a lot of research on the topic, and I believe I understand how the code works.
```
import numpy as np


# Creating a list of numbers, where the index is the decimal number, with the value
# being the amount of 1's that number has in binary.
...
# Return the right Sbox substitution
...
if bestguess == []:
    ...
    # Reading the lengths of the data
    numtraces = np.shape(traces)[0]-1
    numpoint = np.shape(traces)[1]

    # Start of with an empty list of best guess
    bestguess = [0]*16
    
    # For each key-byte
    for bnum in range(0, 16):
        
	# Create more empty arrays
        cpaoutput = [0]*256
        maxcpa = [0]*256
	...
	# Key-byte guess
	    ...
	    # Hypothesis in the same size as the power consumption list
	    # 49 lists (50-1)
            hyp = np.zeros(numtraces)
	    
	    # For number of traces we add the hammer-weight to the hypothesis
	    
            for tnum in range(0, numtraces):
	    	# This is the XOR + Sbox substitution operation
                hyp[tnum] = HW[intermediate(pt[tnum][bnum], kguess)]
		# Returns the number of 1's in the number
		
		# This is also where the current key-byte is important
		# As we are looking for the bnum byte of the input data lists

            #Mean of hypothesis for the key guess.
		...
		# The hypothesis for this trace - the mean hypothesis = the hypothesis difference
                hdiff = (hyp[tnum] - meanh)
		
		# Trace difference
		# The column of this trace number - the mean of all points in trace = the trace difference
	    ...
	    # Calculate the correlation scores. I believe this is the Pearson correlation coefficient
            cpaoutput[kguess] = sumnum / np.sqrt( sumden1 * sumden2 )
	    
	    # Get the key-byte with the highest correlation score
            maxcpa[kguess] = max(abs(cpaoutput[kguess]))

	# Add the best key-byte guess to the overall guess
...
This is essentially the results of a SCA (Side-Channel-Attack). Where the data has been recorded by monitoring physical outputs of a device, such as power consumption.
And the physical ouputs of the device, will be correlated to any operations happening inside, such as cryptographic operations. 

Here, the power consumption has been monitored by an oscilloscope. 

And it gives us the two things we need.

- `The input data (ønskelister.npy)`
and
- `The power consumption (strømforbruk.npy)`

Because we know the type of encryption used, we know roughly what is going in inside the device.

- We know that an XOR is being performed with an unknown number.

- We know that this is passed through a lookup table (S-Box)

- We don't need to check anything it does after that.

- We can assume that the number of 1's on a databus is related to the power consumption. (Not the case on all devices)

- We can now guess what the hypothetical key-byte is for each input byte.

- We know the input data from ønskelister.npy, we know this is XOR'ed with the hypothetical key-byte in the AddRoundKey step of AES encryption. And we know this goes through the S-Box, we can then check how many 1's are in that number.

- We do this for every index-value, with every possible key-byte.

- We assume the number of 1's in the number is related to the power consumption at that time.

- Now we ask if any of these guesses actually ARE RELATED to the power consumption at some point in time.

- We then take the key-byte that is the most likely value at a certain point. And add it to the overall key.


If you haven't heard about the S-Box before, it is there to obscure the relationship between the key and ciphertext. And is part of the second step of AES encryption.
...
I simply combined them all in photoshop using the overlay blending mode, and threw it into Cyberchef:

![](images/luke23_1.png)

Also, the QR is inverted, but still reads well. Awesome.


So what are bit planes?

- Each pixel in an image is represented by a number of bytes. for RGB, we have 3 bytes.
- Lets say we have a fully white image. And lets look at the first pixel
- The byte for R = 255 =  1111 111(1)
- The byte for G = 255 =  1111 111(1)
- The byte for B = 255 =  1111 111(1)
- The numbers in parenthases are the first bit for each byte. Which is the first bit plane.
- Now, changing this bit to 0 will make an invisible difference. As we are changing the value from 255 to 254.
- But if we have a 16x16 pixel image. we now have 16*16 = 256
- 256*3 = 768 bits we can use to store information.
- This can be represented with a 0 on the first bit plane being white, while a 1 on the first bit plane being black.


And just to round this task off. QR codes

The 3 squares are used to identify the version of the QR code, as well as identify size of bits.

![](images/qr.png)
https://en.wikipedia.org/wiki/QR_code#/media/File:QR_Character_Placement.svg Credit to Wikipedia user Bobomath for the image

Each square is a bit, white = 0, black = 1.

We can also see that the QR code fills up unused space.
...
Could have done this in a jiffy myself, but I'm unfortunately busy eating gingerbread.
...
We can see in the code that the positions are encoded with ASN.1. More info here: https://en.wikipedia.org/wiki/X.690#BER_encoding 
...
`0x07` marks the length of the sequence
...
```
SETT r11, 1  ; Incrementer, also max delta y
SETT r12, 0
SETT r13, 0
SETT r14, 0
SETT r15, 2  ; Max delta x
HOPP prep
...
Thanks to the NPST community for being extremely helpful, and creating a really nice environment to discuss CTF's and christmas.

Thanks to PST for organizing this awesome event yearly. Can't wait for Easter.

Special thanks to the writeups:

- myrdyr/Unblvr:        https://github.com/myrdyr/ctf-writeups/blob/master/npst20/README.md (Norwegian)
- WastefulNick:         https://github.com/WastefulNick/npst-2020 (English)
- William solem/Sithis: https://github.com/williamsolem/NPST-2020-Writeup (Norwegian)
- LarsW:                Writeup is set to Secret, and I don't want to share secret link here.
- PJWalstrom:           https://github.com/pjwalstrom/public/blob/main/docs/ctf_pst_2020.md (Norwegian)
- Roy Solberg:          https://blog.roysolberg.com/2021/01/pst-challenge-5 (English)

These were very useful not only for me to be able to do the tasks I did not do, but also to help me find resources and information I otherwise would not have found.


Thanks for reading! Send me a message if you discover anything wrong. Typos/mistranslations/Anything wrong.
